...
class MonthsController < ApplicationController
  ...
  def show
    ...
    @totalTimeIn = @month.days.sum(:timein)
    @totalTimeOut = @month.days.sum(:timeout)
    @totalCashAmount = @month.days.sum(:totalamount)
    @totalDailyWorkingHours = @month.days.sum(:dailyworkhours)
  ...
  def create
    ...
    @month = @year.months.build(params[:month].merge(:user_id => @year.user.id))
    @month.save


    respond_to do |format|
      if @month.save
        format.html { redirect_to @month.year }
